% This script performs the randomization test of the mean voltage difference
% between the high and low WM load conditions in sliding windows, on grand average ERPs.
...
% width of the window of interest, in samples:
win = 10;

% step size, in samples, between successive windows:
step = 2;
...
for u = 0:step:(lenERP-win)      
    ...
    S.FaceAlpha = 0.25;
    patch(S)
    title 'Randomization test of mean voltage within the window at Cz'
    legend ('low WM load', 'high WM load', 'p-value')
    ...
    % report the Monte-Carlo estimate of the p-value:
...
title('UNCORRECTED RANDOMIZATION P-VALUES')
